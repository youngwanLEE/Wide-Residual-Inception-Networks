--  Wide-Residual-Inception-Network
--  This is an implementation of the wide residual inception networks
--  authorized by Youngwan Lee
...
local function createModel(opt)
   ...
   -- Wide-Residual-Inception Networks for stride 2
   local function InceptionResnet_v4(nInputPlane, nOutputPlane,out2a, out2b_3, out2c_3_a, out2c_3_b,stride) 
     ...
     local fs2a = {out2a};                                    			fs2a[0] = nInputPlane 
     local fs2b = {out2b_3};                                    		fs2b[0] = fs2a[1]
     local fs2c = {out2c_3_a,out2c_3_b};                                fs2c[0] = fs2a[1]
     local fs2  = {nOutputPlane};  --local fs2  = {384}; 
           fs2[0]  = fs2a[#fs2a] + fs2b[#fs2b] + fs2c[#fs2c] 
     
     local stem  = nn.Sequential():add(BNReLUConv(fs2a[0], fs2a[1], 1, 1, stride, stride, 0, 0))
     local branch1 = Tower(
                      {
                         BNReLUConv(fs2b[0], fs2b[1], 3, 3, 1, 1, 1, 1)
                      }
     ...
     local net = nn.Sequential()                                 
     net:add(nn.ConcatTable():add(shortcut)
                             :add(stem)
                             )
     net:add(nn.CAddTable()) 
   ...
   do
      ...
      model:add(layer(wide_basic, nStages[2], nStages[3], n-1, 2)) -- Stage 2 (spatial size: 16x16)
      
      model:add(InceptionResnet_v2(nStages[3])) -- Inception Module
      
      model:add(InceptionResnet_v4(nStages[3],nStages[4],128,256,128,256,2))
      model:add(InceptionResnet_v4(nStages[4],nStages[4],256,128,128,256,1))      
